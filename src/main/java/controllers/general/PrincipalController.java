...
public class PrincipalController extends StackPane {
    ...
    // Agregar la vista a la colecci�n
    ...
    // Setea la vista, con validaciones en caso de que no est� cargado el FXML
    public boolean setScreen(final String name) {
        if (screens.get(name) != null) {   // El fxml est� cargado
            final DoubleProperty opacity = opacityProperty();

            if (!getChildren().isEmpty()) {    // Si hay m�s de uno, animamos
    ...
    // Elimina la vista de la colecci�n
    public boolean unloadScreen(String name) {
        if (screens.remove(name) == null) {
            System.out.println("La vista no exist�a. \n");
            return false;
        } else {
            return true;
        }
